...
const Contact = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault(); // prevent the default refresh behaviour
        ...
        >
          ...
          <form className='flex flex-col gap-5' onSubmit={handleSubmit}>
            ...
            <div className='flex flex-col'>
              ...
              >
                <option className="text-gray-400" value="" disabled>Something in Mind?</option>
                <option className="text-black" value="Web Developement">Web Developement</option>
                <option className="text-black" value="Software Developement">Software Developement</option>
                <option className="text-black" value="Mobile Application">Mobile Application</option>
...

            {
              formData.service && formData.service !== "Other" && (
                <div className='flex flex-col'>
                  <label className='mb-1'>Your Budget <span className='text-red-500'>*</span> </label>
                  <input
                    type="text"
                    name='budget'
            ...
            <div className='flex flex-col'>
              <label className='mb-1'>Your Idea <span className='text-red-500'>*</span> </label>
              <textarea
                name='idea'
                rows={5}
            ...
            {
              status && (
                <p className={`text-sm ${status === "success" ? "text-green-400" : status === "failed" ? "text-red-500" : "text-yellow-400"}`}>{status === "sending" ? "sending..." : status === "success" ? "Message sent successfully✅" : "Something went wrong❌"}</p>
              )
            }
